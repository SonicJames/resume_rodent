...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (jobUrl) jobForm.elements["jobUrl"].value = jobUrl;
  if (jobTitle) jobForm.elements["jobDescription"].value = "Job Title: " + jobTitle;
  if (resumeText) jobForm.elements["resumeText"].value = resumeText;
  if (jobUrl) setStatus("Job loaded from Claude. Add your resume and click Start analysis.");
});

const setStatus = (message) => { statusLog.textContent = message; };
...
const startAnalysis = async (event) => {
  ...
  try {
    const requestOptions = (resumeFile && resumeFile.name)
      ...
      : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jobUrl, jobDescription, resumeText }) };
    ...
    state.conversation = [
      { role: "user", content: "Please analyze this resume and job posting. Job URL: " + (jobUrl || "(not provided)") + ". Job description: " + (jobDescription || "(not provided)") + "." },
...
const sendChatMessage = async (event) => {
  ...
  try {
    const response = await fetch("/api/chat", {
      ...
      body: JSON.stringify({ jobUrl: state.jobUrl, jobDescription: state.jobDescription, resumeText: state.resumeText, conversation: state.conversation })
    });
    const data = await response.json();
